...
const CaptainDetails = () => {
  ...
  const firstName =
    captain?.fullname?.firstname || captain?.fullName?.firstName || "Captain";
  const lastName =
    captain?.fullname?.lastname || captain?.fullName?.lastName || "";
  const name = `${firstName} ${lastName}`.trim();

  const rawEarnings = captain?.earnings ?? 0;
  const formattedEarnings =
    typeof rawEarnings === "number"
      ? rawEarnings.toLocaleString("en-IN")
      : rawEarnings;
